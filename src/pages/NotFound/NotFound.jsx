  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#fbfdfe] px-5 py-10 text-slate-900">
      ...
      {floatingIcons.map(({ Icon, className }, index) => (
        <Icon
          key={index}
          className={`pointer-events-none absolute text-emerald-300 ${className}`}
          aria-hidden="true"
        />
      ))}
      ...
      <section className="relative z-10 flex w-full max-w-3xl flex-col items-center text-center">
        <div className="relative mb-10">
          <div className="flex h-40 w-56 items-center justify-center rounded-[22px] border border-emerald-50 bg-white/85 shadow-[0_24px_70px_rgba(16,185,129,0.16)] backdrop-blur">
            <FaShoppingCart className="text-7xl text-emerald-400" aria-hidden="true" />
          </div>

          <div className="absolute -right-8 -top-6 grid h-24 w-24 place-items-center rounded-full border-[8px] border-white bg-emerald-500 text-2xl font-black text-white shadow-[0_12px_24px_rgba(22,163,74,0.28)]">
            404
          </div>

          <div className="absolute -bottom-12 left-1/2 flex -translate-x-1/2 items-start gap-4 text-emerald-400">
        ...
        <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
          <a
            href="/"
            className="inline-flex h-14 items-center justify-center gap-3 rounded-xl bg-emerald-600 px-8 text-base font-extrabold text-white shadow-[0_10px_20px_rgba(22,163,74,0.28)] transition hover:-translate-y-0.5 hover:bg-emerald-700 focus:outline-none focus:ring-4 focus:ring-emerald-200"
          >
